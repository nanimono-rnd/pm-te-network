...
Genuine-symmetric & 13 & 7.4\% & 39.2 & Genuine with $|\text{AR}| < 0.1$ \\
Event-amplified & 9 & 5.1\% & 21.3 & Both regimes; stronger during events \\
Hierarchical & 8 & 4.5\% & 46.4 & Within-concept pairs (e.g., core/headline CPI) \\
\cmidrule{2-4}
\textit{Subtotal: survive} & \textit{109} & \textit{62.0\%} & & \\
\midrule
...
Unclassified & 1 & 0.6\% & --- & Insufficient data \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
& Edge & Pers. & Rate (near) & Rate (base) & Ratio \\
\midrule
\multicolumn{6}{l}{\textit{Panel A: Information Leaders (pre-FOMC rate shown)}} \\
...
\midrule
\multicolumn{6}{l}{\textit{Panel B: Common-Shock Responders (post-FOMC rate shown)}} \\
...
\bottomrule
\multicolumn{5}{l}{\footnotesize Full ranking of all 16 nodes with edges in the identified network. Three of 19 composites}\\
\multicolumn{5}{l}{\footnotesize (fed\_gov\_confidence, jobless\_claims, potus\_social) lack sufficient edges for ranking.}
\end{tabular}
\end{table}
...
The clean hierarchy reveals that markets extract more directional information from \textit{how} the Federal Reserve deliberates than from \textit{what} it decides. \texttt{fomc\_dynamics}---contracts referencing FOMC deliberations, dissent patterns, and communication strategy---ranks as the top information originator, \textit{above} the rate decision itself. This extends the central bank communication literature \citep{gurkaynak2005actions,hansen2016shocking} by showing that FOMC ``extra'' information is the primary driver of cross-market belief dynamics.
...
Placebo (1,000 calendars) & $p = 0.187$ & Limited power; triangulation is primary \\
\bottomrule
\end{tabular}
\end{table}
